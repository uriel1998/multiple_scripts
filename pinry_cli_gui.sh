...
PINRY_BIN="${PINRY_BIN:-$(command -v pinry || true)}"
EXIFTOOL_BIN="$(command -v exiftool || true)"
AI_ALT_TEXT_BIN="$(command -v ai_gen_alt_text.sh || true)"
...
process_file () {
    ...
    if [[ "$exists" == "0" ]];then
		if [ -n "${EXIFTOOL_BIN}" ];then
			taglist=""
			taglist=$("${EXIFTOOL_BIN}" "${1}" | grep Subject | awk -F ':' '{print $2}' | xargs )
			# xargs trims beginning, ending whitespace from tags
			description=$("${EXIFTOOL_BIN}" "${1}" | grep Comment | awk -F ':' '{print $2}')
		fi
		# skip assets
		if [[ "$taglist" != *"asset"* ]];then 
			TEMPFILE3=$(mktemp)    
			convert "${file}" -resize "600x600" "${TEMPFILE3}"
			if [ -n "${AI_ALT_TEXT_BIN}" ] && [[ "${description}" == "" ]];then 
				description=$("${AI_ALT_TEXT_BIN}" "${TEMPFILE3}" "Describe this in one sentence using no punctuation and not mentioning it is an image" | sed -e 's/ "/ “/g' -e 's/" /” /g' -e 's/"\./”\./g' -e 's/"\,/”\,/g' -e 's/\."/\.”/g' -e 's/\,"/\,”/g' -e 's/"/“/g' -e "s/'/’/g" -e 's/ -- /—/g' -e 's/(/❲/g' -e 's/)/❳/g' -e 's/ — /—/g' -e 's/ - /—/g'  -e 's/ – /—/g' -e 's/ – /—/g' | hxunent -f)
			fi
			...
			if [[ -z "$result" ]]; then
				...
				if [ -n "${EXIFTOOL_BIN}" ] && [[ "${desc}" != "" ]];then
					"${EXIFTOOL_BIN}" -preserve -overwrite_original_in_place -comment="${desc}" "${file}"
				fi
				file="$(realpath "${file}")"
				outstring=$(printf "%q add %q --board %q --tags %q --description %q" "${PINRY_BIN}" "${file}" "${board}" "${tags}" "${desc}")
				echo "${outstring}"
				"${PINRY_BIN}" add "${file}" --board "${board}" --tags "${tags}" --description "${desc}"
...
if [[ -z "${PINRY_BIN}" ]]; then
    echo "pinry command not found on PATH. Set PINRY_BIN or install pinry."
    exit 1
fi
